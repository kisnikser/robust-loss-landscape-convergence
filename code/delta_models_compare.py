...
@hydra.main(version_base=None, config_path='configs/CIFAR10', config_name='conv_layers_num')
def main(config: DictConfig):
    ...
    for param in config['model']['variable_param_grid']:
        ...
        param_to_final_loss[param] = np.mean(valid(model, criterion, dataloader, device=config['device'])[0])
    ...
    for i in param_to_deltas.keys():
        arr = param_to_deltas[i][:-1]*np.arange(1, len(param_to_deltas[i]))
        arr = smooth(arr, config['visualize']['smooth_window'])
        axs[0][0].plot(arr, label = f'{i}')
        axs[1][0].plot(arr/param_to_final_loss[i], label = f'{i}')
    ...
    axs[1][1].plot(x, y/np.array(list(param_to_final_loss.values())))
    axs[1][1].set(
        xticks = x,
        ylabel = 'avg($k\Delta_k$)/$\mathcal{L}$',
        xlabel = config['model']['variable_param']
    )
